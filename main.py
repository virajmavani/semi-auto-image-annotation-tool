...
class MainGUI:
    def __init__(self, master):
        ...
        self.openDirBtn = Button(
            self.ctrlPanel, text="Open Dir", command=self.open_image_dir
        )
        ...
        self.previousBtn = Button(
            self.ctrlPanel, text="<-- Previous", command=self.open_previous
        )
        ...
        self.radioBtnAuto = Radiobutton(
            self.ctrlPanel, text="Auto", variable=self.autoSuggest, value=1
        )
        self.radioBtnAuto.grid(row=7, column=0, sticky=W + E)
        self.radioBtnManual = Radiobutton(
            self.ctrlPanel, text="Manual", variable=self.autoSuggest, value=2
        )
        ...
        self.mb = Menubutton(
            self.ctrlPanel, text="COCO Classes for Suggestions", relief=RAISED
        )
        ...
        self.addCocoBtnAllClasses = Button(
            self.ctrlPanel, text="Add All Classes", command=self.add_all_classes
        )
        self.addCocoBtnAllClasses.grid(columnspan=2, sticky=W + E)

        # options to add different models
        self.mb1 = Menubutton(
            self.ctrlPanel, text="Select models from here", relief=RAISED
        )
        self.mb1.grid(columnspan=2, sticky=W + E)
        self.mb1.menu = Menu(self.mb1, tearoff=0)
        self.mb1["menu"] = self.mb1.menu

        self.addModelBtn = Button(
            self.ctrlPanel, text="Add model", command=self.add_model
        )
        ...
        self.listBoxNameLabel = Label(self.listPanel, text="List of Objects").pack(
            fill=X, side=TOP
        )
        ...
        self.clearAllBtn = Button(
            self.listPanel, text="Clear All", command=self.clear_bbox
        )
        self.clearAllBtn.pack(fill=X, side=TOP)
        self.classesNameLabel = Label(self.listPanel, text="Classes").pack(
            fill=X, side=TOP
        )
        self.textBox = Entry(self.listPanel, text="Enter label")
        self.textBox.pack(fill=X, side=TOP)
        self.addLabelBtn = Button(
            self.listPanel, text="+", command=self.add_label
        ).pack(fill=X, side=TOP)
        self.delLabelBtn = Button(
            self.listPanel, text="-", command=self.del_label
        ).pack(fill=X, side=TOP)
        ...
        self.enterthresh = Button(
            self.listPanel, text="Set", command=self.changeThresh
        ).pack(fill=X, side=TOP)
        ...
        for idxcoco, label_coco in enumerate(self.cocoLabels):
            ...
            self.mb.menu.add_checkbutton(
                label=label_coco, variable=self.cocoIntVars[idxcoco]
            )
    ...
    def available_models(self):
        ...
        model_categ = [
            dir_
            for dir_ in os.listdir(self.models_dir)
            if os.path.isdir(os.path.join(self.models_dir, dir_))
        ]
    ...
    def load_image(self, file):
        ...
        self.imageIdxLabel.config(
            text="  ||   Image Number: %d / %d" % (self.imageCur, self.imageTotal)
        )
    ...
    def save(self):
        if self.filenameBuffer is None:
            w, h = self.img.size
            self.writer = Writer(
                os.path.join(self.imageDirPathBuffer, self.imageList[self.cur]), w, h
            )
            ...
            for idx, item in enumerate(self.bboxList):
                ...
                self.annotation_file.write(
                    ...
                    + ","
                    + ",".join(map(str, self.bboxList[idx]))
                    + ","
    ...
    def mouse_release(self, event):
        ...
        self.objectListBox.insert(
            END, "(%d, %d) -> (%d, %d)" % (x1, y1, x2, y2) + ": " + str(self.currLabel)
        )
    ...
    def zoom_view(self, event):
        try:
            ...
            self.zoomImgCrop = self.zoomImg.crop(
                ((event.x - 25), (event.y - 25), (event.x + 25), (event.y + 25))
            )
            self.zoomImgCrop = self.zoomImgCrop.resize((150, 150))
            self.tkZoomImg = ImageTk.PhotoImage(self.zoomImgCrop)
            self.zoomImgId = self.zoomcanvas.create_image(
                0, 0, image=self.tkZoomImg, anchor=NW
            )
    ...
    def automate(self):
        ...
        for idx, (box, label, score) in enumerate(zip(boxes[0], labels[0], scores[0])):
            ...
            o1 = self.canvas.create_oval(
                b[0] - 3, b[1] - 3, b[0] + 3, b[1] + 3, fill="red"
            )
            o2 = self.canvas.create_oval(
                b[2] - 3, b[1] - 3, b[2] + 3, b[1] + 3, fill="red"
            )
            o3 = self.canvas.create_oval(
                b[2] - 3, b[3] - 3, b[2] + 3, b[3] + 3, fill="red"
            )
            o4 = self.canvas.create_oval(
                b[0] - 3, b[3] - 3, b[0] + 3, b[3] + 3, fill="red"
            )
            self.bboxPointList.append(o1)
            self.bboxPointList.append(o2)
            self.bboxPointList.append(o3)
